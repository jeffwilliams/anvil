...
var keyActions = []keyAction{
	...
	},
	{
		...
		func(gtx layout.Context, e *editable, ev *key.Event, args []string, state actionProcessingState) (result keyHandlerResult) {
			...
			etx := e.eventContextOfIndex(gtx, start)

			var flags CmdFlags
			if slices.Contains(args, "clear-errors") {
				flags = CmdFlagClearErrorsWindow
			}

			e.adapter.execute(e, gtx, text, nil, etx, flags)
			return
		},
	},
	{
	...
	},
	{
		...
		func(gtx layout.Context, e *editable, ev *key.Event, args []string, state actionProcessingState) (result keyHandlerResult) {
			...
			t, rng := e.textObjectForExecutionAt(ndx)
			if t != "" {
				...
				e.adapter.execute(e, gtx, t, nil, etx, flags)
			}
			result.clearRecentlyTypedText.Set(true)
	...
	},
	{
		...
		func(gtx layout.Context, e *editable, ev *key.Event, args []string, state actionProcessingState) (result keyHandlerResult) {
			...
			savedSelections := e.copySelections()

			e.selectRecentlyTypedText()

			result.handled = !e.selectionsEqual(savedSelections)
			return
		},
	},
	{
	...
	},
	{
		...
		func(gtx layout.Context, e *editable, ev *key.Event, args []string, state actionProcessingState) (result keyHandlerResult) {
			...
			var etx *EventContext
			if len(e.CursorIndices) > 0 {
				etx = e.eventContextOfIndex(gtx, e.CursorIndices[0])
			}
			e.adapter.execute(e, gtx, text, nil, etx, 0)
			return
		},
	},
	{
	...
	{
		"del-float", "Close the currently focused float",
		[]string{},
		func(gtx layout.Context, e *editable, ev *key.Event, args []string, state actionProcessingState) (result keyHandlerResult) {
			editor.DelFocusedFloat()
			return
		},
	},
}
